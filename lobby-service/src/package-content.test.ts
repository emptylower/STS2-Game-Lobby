...
test("service package uses exact production allowlist and deterministic temporary outputs", () => {
  ...
  try {
    ...
    assert.equal(packageJson.version, "0.6.1-alpha.2");
    assert.equal(packageLock.version, "0.6.1-alpha.2");
    assert.equal(packageLock.packages?.[""]?.version, "0.6.1-alpha.2");
...
test("release sources pin client and service 0.6.1-alpha.2 while preserving older fixtures", () => {
  ...
  assert.equal(servicePackage.version, "0.6.1-alpha.2");
  assert.equal(serviceLock.version, "0.6.1-alpha.2");
  assert.equal(serviceLock.packages?.[""]?.version, "0.6.1-alpha.2");
  assert.equal(clientManifest.version, "0.6.1-alpha.2");
  assert.match(clientProject, /<Version>0\.6\.1-alpha\.2<\/Version>/);
