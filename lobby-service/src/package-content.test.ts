...
test("service package uses exact production allowlist and deterministic temporary outputs", () => {
  ...
  try {
    ...
    assert.equal(packageJson.version, "0.6.3-alpha.1");
    assert.equal(packageLock.version, "0.6.3-alpha.1");
    assert.equal(packageLock.packages?.[""]?.version, "0.6.3-alpha.1");
    assert.match(readFileSync(join(packageDir, "README.md"), "utf8"), /v0\.6\.2/);
...
test("release sources pin client and service 0.6.3-alpha.1 while preserving older fixtures", () => {
  ...
  assert.equal(servicePackage.version, "0.6.3-alpha.1");
  assert.equal(serviceLock.version, "0.6.3-alpha.1");
  assert.equal(serviceLock.packages?.[""]?.version, "0.6.3-alpha.1");
  assert.equal(clientManifest.version, "0.6.3-alpha.1");
  assert.match(clientProject, /<Version>0\.6\.3-alpha\.1<\/Version>/);
  assert.match(clientProject, /<AssemblyVersion>0\.6\.3\.0<\/AssemblyVersion>/);
