...
test("release sources pin client and service 0.6.1 while preserving older fixtures", () => {
  ...
  assert.equal(servicePackage.version, "0.6.1");
  assert.equal(serviceLock.version, "0.6.1");
  assert.equal(serviceLock.packages?.[""]?.version, "0.6.1");
  assert.equal(clientManifest.version, "0.6.1");
  assert.match(clientProject, /<Version>0\.6\.1<\/Version>/);
