...
- **创建日期**：2026-07-26
- **状态**：全部五期已实施完成（2026-07-27），待用户实机验收
- **实施记录**：见 `docs/superpowers/plans/` 下第 1–2 期、第 3 期、第 4 期、第 5 期四份计划文末的「实际落地记录」
...
1. **中文描边宽度：已定稿为 2px（原 3px）。**

   离线截图硬件（`0cc56de` 引入的 `LanConnectHudLegibilityScreenshotTests`，产出 `outline-width-grid.png`，
   SHA-256 `5fd25238d8883089bedc2fa9001d58324f0054069254637da538b2c0a58059b4`）已人工审阅。该测试渲染固定的
   `ark-pixel-10px-proportional-zh_cn.otf`（仓库自带的测试字体，10px 像素字体），**不是**游戏实际使用的字体，
   因此结论分两部分：

   - **已证实，且与具体字形无关**（这是合成层面的结论，不是字形层面的）：描边是必需品，不是装饰。在
     `#d9cba8` 亮色羊皮纸背景带上，`outline 0` 时中英文均近乎不可见。
   - **未证实**：3px 描边在游戏实际字体下是否具体糊住中文笔画——像素字体的 1px 量化笔画不能类推到游戏字体。

   **据此定稿的推理：** Godot 的描边从字形边缘向外生长、渲染于字形之下。13px 字号下，中文字内部的
   "字腔"（笔画围出的空白）通常只有 1–2px 宽，3px 描边很可能把它们填死。硬件截图里 `outline 3` 的中文列
   已明显比 `outline 2` 更重、更糊；而 `outline 2` 在最亮的背景带上依然清晰可读。既然 2px 已经够用，就没有
   理由继续承担 3px 糊笔画的风险。**`OutlineSize` 已锁定为 2**（`LanConnectHudLegibility.cs`），对应测试
   断言已从 `>= 2` 收紧为 `== 2`。

   **仍然悬而未决：** 游戏实际字体下的最终观感尚未过人眼——离线硬件收窄了风险，没有消除它。这一项留给
   用户的验收环节，在真实 1920×1080 桌面与安卓竖屏各取一张截图确认。

2. **静息底板 0.35 的充分性：已定稿为 0.45（原 0.35）。**

   离线截图硬件产出的 `rest-plate-alpha-grid.png`（SHA-256
   `a14370abbd6e7af130e8c3626ac74e9d0f5a7c7cdb4454622204e1e628655ed0`）已人工审阅：在最亮的 `#d9cba8`
   背景带上，0.45 明显优于 0.35。描边承担了大部分可读性责任，底板只是加一层余量——这一点与所用字体无关，
   截图结论可直接采信。`HoverPlateColor (0.7)` 与 `PressedPlateColor (0.85)` 未受影响，仍严格大于新的静息值。
   `LanConnectHudLegibility.Plate()` 里的 5px 圆角与 10/6px 内边距本轮未评估，仍待后续实机确认。

   **两张截图本身不入库**，按既定约定仅以上述 SHA-256 存档以供追溯。
