...
Create fixed `.bin + .json` vectors for both complete STS2 messages. Capture the opaque vanilla body through the target v0.111.0 production serializer, then freeze its SHA-256 and independently review it with a separate parser and adjacent byte-map; the expected fixture must never be regenerated by the test. InitialGameInfo requires session selection, forbids roster/rejection, uses selected session protocol 1, and validates host identity. Join request requires peer offer, forbids roster/rejection, uses session protocol 0, and validates that the offer belongs to the transport sender.
...
Add a fixed complete `PlayerJoinedMessage` `.bin + .json` vector, including the vanilla body boundary, session-selection entry, full roster entry, and exact `2/3-bit` assertions. Its opaque vanilla body may be captured from the target production serializer, but the adjacent byte-map, parser checks, boundary assertions and SHA-256 must be independently authored; the test must not regenerate expected bytes.
...
Required smoke cases for this user-authorized alpha scope (Windows-specific real-device runs are waived; Windows build/package checks remain mandatory):
...
`GO` for the no-Ritsu `tail_v1` path requires every revised in-scope criterion, both mismatch directions, readiness failures, and Android evidence. The all-Ritsu path receives its own verdict: if the third-party RitsuLib cannot initialize on Android, record that path as externally BLOCKED and keep LAN Connect fail-closed; do not fork, patch, bundle, or silently downgrade RitsuLib. This external blocker does not invalidate the completed no-Ritsu path, but an all-Ritsu release claim remains NO-GO.
...
- LAN container/entry/sidecar golden bytes must be independently authored. Complete STS2 message fixtures may capture opaque vanilla bodies from the pinned production serializer, but require a separate parser, independently authored byte-map/boundary assertions and fixed SHA-256; tests may not regenerate expected bytes.
