...
当前正式版为 `0.6.3`，客户端与 lobby-service 版本号已同步对齐。本版可从 GitHub Release 或 Steam 创意工坊（条目「游戏大厅」，2026-09-24 更新至 `0.6.3`）获取。同房玩家必须统一客户端与游戏版本，安装或更新后必须完整重启游戏。
...
`0.6.0` 收敛了 `0.5.6-rc1`~`rc4` 与 `0.6.0-alpha.1`~`alpha.9` 全部测试候选。完整说明见 `docs/RELEASE_NOTES_V0.6.0_ZH.md`。
...
- 加入前会比较双方实际使用的 ModelId 线上编码。内容 MOD 组合不同并改变 net-id 表或位宽时，会明确提示不一致并拒绝加入，而不是进入后黑屏或一方卡在等待页；这是预期行为。
- `affects_gameplay: false` 只表示该 MOD 不进入原版 `idDatabaseHash`，不保证它不会占用 ModelId。新签名会补上这层检查；签名缺失或读取失败时仍允许加入。
- 默认兼容配置为 strict。普通 MOD 差异的显式 relaxed 入口不能跳过真实线上编码或游戏版本不一致。
...
- 槽位接管后的踢出会针对当前占用者，不再永久封禁槽位原主人。列表过期时服务端拒绝操作。
- “放弃多人存档”确认弹窗为选项区保留固定的最小可视高度，横屏和竖屏下都能完整显示“删除存档”与“保留存档”。

**服务端与稳定性**

- lobby-service 不再因开局加载暂时超过房间心跳窗口而删除仍有活跃房主的中继；中继空闲超时仍会在游戏连接真正停止后回收资源。
- Tail 房主控制通道会提交建房时冻结的客户端版本与 capability digest，平台/房间消息和玩家控制绑定不再因协议身份缺失而加载失败。
- lobby-service 不再改变区分大小写的 Base64URL WireCache 签名。
- 遇到启动失败、加入失败、黑屏、等待页卡住或 MOD 误判，请保留两端完整日志；Android 按下方取证步骤操作。

### Android 取证

1. 从 launcher 导出本次启动的完整 `sts2*.log`。客户端会把 `patch_diag` 同步写入普通日志，私有目录不可读时仍可定位最后一个 stage 与 patch ID。
...
`0.6.0` consolidates every candidate from `0.5.6-rc1` through `0.6.0-alpha.9`. Full notes (Chinese): `docs/RELEASE_NOTES_V0.6.0_ZH.md`.

**"Lobby never appears" is fixed**
...
- LAN Connect does not detach, invoke, or restore private RitsuLib Harmony patches, and does not maintain a RitsuLib fork.
- All macOS and Android RitsuLib peers must use official v0.5.13 or newer; v0.5.12 should not be used.
- Direct IP is compat-only. Interoperability with historical `0.3.x`-`0.5.x` clients is outside the release gate.

**Pre-join wire-encoding check**

- The join flow compares the actual ModelId wire encoding. Content-MOD sets that change net-id tables or bit widths are intentionally rejected before a black screen or stuck waiting room.
- `affects_gameplay: false` only excludes a MOD from vanilla `idDatabaseHash`; it does not guarantee that the MOD takes no ModelIds. The signature covers that gap and remains fail-open when unavailable.
- The shipped profile is `strict`. Explicit relaxed handling for ordinary MOD differences cannot bypass a genuine wire-signature or game-version mismatch.
...
- RitsuLib sidecar activation waits for both the player-control binding and the ENet connection.
...
- A kick after slot takeover targets the current occupant instead of banning the original owner. Stale list actions are rejected.
- The abandon-multiplayer-save confirmation reserves a stable minimum viewport for its choices, keeping both Delete Save and Keep Save fully visible.

**Service and stability**

- Lobby-service keeps an authenticated active relay alive when a run load exceeds the room-heartbeat window; relay idle cleanup still reclaims the session after traffic stops.
- Tail host control connections submit the frozen client version and capability digest.
...
1. Export the launcher's complete `sts2*.log` for the failed startup. The client mirrors `patch_diag` events into ordinary logs, preserving the last stage and patch ID when private files cannot be read.
