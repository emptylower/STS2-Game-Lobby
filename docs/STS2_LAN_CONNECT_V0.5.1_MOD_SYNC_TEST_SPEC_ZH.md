...
| 0.108.0 | 历史 Steam `public-beta`，build `24032229`，macOS manifest `1977841934321910790` | 2026-07-18 官方 appinfo 确认当前无分支指向该 build；已登录 Steam 控制台常规重试两次，并临时启用内建 `@bClientTryRequestManifestWithoutCode=1` 精确重试一次，三次均返回 manifest ACL `Access Denied`；开关已恢复为 `0`，仍待合法产物 |
