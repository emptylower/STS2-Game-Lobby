...
>
> 容器化部署与运维步骤自 v0.5.4 起未变，下文的 v0.5.4 说明继续适用：部署仍只需要 `lobby-service` 一个容器。`0.6.1` 与 `0.6.0` 代码功能等价（`0.6.1-alpha.2` 起服务端代码未再变过），只对齐版本号；仍在 `0.6.0` 的节点属于可选升级。以前的“双服务”栈（lobby-service + server-registry + postgres）仅作历史运维参考。
