...
服务器列表能力标识增量（2026-07-18）：测试节点 `http://101.35.217.99:8788` 由客户端固定注入、规范化去重并始终排在第一位；服务端在 `/probe` 和 `/peers/metrics` 声明 MOD 同步协议、运行时开关与最低客户端版本 `0.5.1`，客户端仅在实时能力完整且开启时显示“支持 0.5.1+ MOD 同步”，旧服务端不显示但继续兼容。TDD 覆盖空发现列表、尾斜杠去重、置顶优先级、metrics JSON DTO、管理开关实时变化和 Godot 卡片双标识。完整 `verify-release.sh` 通过：lobby-service 433/433、xUnit 670 通过及 1 个既有双客户端原型跳过、GdUnit 226/226、客户端构建 0 警告/0 错误。客户端与服务端各两次独立打包 `diff -qr` 为空；使用 `rsync -a --checksum --delete` 同步发布镜像后再次比较为空。客户端 ZIP SHA-256 为 `082fead8502cae6d278aa87dfe90bb541f0235e4384b3611d558a0fc65e2b035`，服务端 ZIP SHA-256 为 `f8f89cedc6d418b3045fee0910ca376e4df5aa17d7cd73d67de2b2aa70bafb5f`。该增量不改变 gameplay MOD 范围、Workshop 唯一下载来源、游戏版本硬拦截或 relaxed 边界；Phase 8 尚缺的外部实机条件仍未满足。
