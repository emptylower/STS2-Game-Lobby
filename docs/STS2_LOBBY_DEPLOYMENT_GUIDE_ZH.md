...
> 当前正式版为 **v0.6.1**，正式包见 [GitHub Release v0.6.1](https://github.com/emptylower/STS2-Game-Lobby/releases/tag/v0.6.1)（附带 `sts2_lobby_service.zip`）。
>
> 本文的部署与验证步骤自 v0.5.4 起未变，下文的 v0.5.4 说明继续适用；v0.6.1 的差异集中在下一节「v0.6.1 升级说明」。去中心化节点网络、富聊天、AI 语义审核和 gameplay MOD 私有预检保持不变。如果你从 v0.3.x 升级，请同时阅读文末的 v0.4.0 架构迁移要点。

## v0.6.1 升级说明

- `0.6.1` 与 `0.6.0` **代码功能等价**（`0.6.1-alpha.2` 起服务端代码未再变过），只对齐版本号。已部署 `0.6.0` 或任意 `0.6.1-alpha.x` 的节点属于可选升级；不升级不影响新协议房间的运行——协议行为主要由客户端与门禁字段决定，服务端字段自 `0.6.1-alpha.1` 起就已支持。
- 本轮客户端改动集中在新协议房间的载体切换（`native_bus_v1`，与 RitsuLib 解耦）和存档 / 续局修复，服务端无需额外配置即可支持。
- v0.6.1 Release 不是 pre-release 且附带 `sts2_lobby_service.zip`，因此**已开启自动更新的节点会自动升级到 `0.6.1`**（服务端自动更新只接受非 pre-release 且带该资产的 Release，且要求 `x.y.z` 格式的版本号）。
