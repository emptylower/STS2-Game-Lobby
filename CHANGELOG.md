...
- SL、保存退出后读档与延迟保存保留原房间冻结的协议。绑定缺失、不完整或校验失败时在建房前停止并提示恢复配置，避免新协议存档静默降级到兼容协议；真实旧版 schema 0/1 兼容记录仍可恢复。
- 配置写入采用同目录临时文件替换，保留有效备份，主配置损坏时恢复备份；两份都不可用时进入只读保护，防止空配置覆盖续局绑定。
- 修复 `0.6.2` 加入兼容协议房间错误要求新协议消息 ID，以及桌面房主开局消息被 JIT 内联绕过位宽补丁造成客机解码错误、房主等待的问题。
- 修复新协议房主昵称同步竞态；MOD 预检弹窗跟随大厅主题，并去除重复的取消按钮。

### Compatibility

- 同房玩家统一更新客户端 `0.6.3` 并完整重启游戏；wire 协议不变，tail 房间 `minimumClientVersion` 保持 `0.6.2-alpha.1`。
- lobby-service 相对 `0.6.2` 仅版本号同步，已有 `0.6.2` 服务端无需为本次存档修复升级；正式 Release 可能被已启用自动更新的节点采纳。
- 不能自动恢复已被旧版本覆盖且没有备份的原协议记录。更新前请备份游戏存档与联机配置。

### Validation and known issues

- `0.6.3-alpha.3` 在 Mac Steam + Android AVD、游戏 `0.111.0`、魔仙堡测试服完成两种协议的首怪、SL 后下一房间及保存退出后读档；新协议携带 RitsuLib `0.5.13`，未出现协议降级。本正式版保持该候选的行为实现。
- 实测出现两次严重丢包后断线，具体网络根因尚未确定；另有候选连接切换时误报 `ModMismatch`，自动重试后成功。握手绑定生命周期是日志与代码分析推测的原因，尚未经故障注入验证。本版未修复这些问题，核心回归通过不代表整体验收全程无异常。

## [0.6.3-alpha.3] - 2026-09-23

`0.6.3-alpha.3` 本地候选版，尚未公开发布。客户端与 lobby-service 版本号同步；修复均在客户端，服务端仅版本号变化，不改 wire 协议，tail 房间 `minimumClientVersion` 仍为 `0.6.2-alpha.1`。

### Fixed

- 多人续局恢复时严格读取原房间冻结的协议选择。原生载入按钮与快速载入入口在游戏建房前校验绑定；绑定缺失、字段不完整或协议校验失败时停止建房并提示恢复配置，避免把新协议存档静默变成“兼容旧版 Mod”房间；确实由旧版 schema 0/1 创建的房间继续按旧协议恢复。
- 存档事件、房间关闭后的延迟保存和“重开一局”写入使用原房间冻结的协议，并按存档键或当前网络会话校验归属，避免在保存时写入推测出的旧协议。
- 配置文件改为先写同目录临时文件、保留上一份有效备份；主文件损坏时恢复备份并保存损坏副本。主文件与备份都不可用时进入只读保护并阻止联机，仍可打开大厅导出诊断，不会用空配置覆盖续局绑定。

## [0.6.3-alpha.2] - 2026-09-19

`0.6.3-alpha.2` 预发布（仅 GitHub pre-release，不更新 Steam 创意工坊；当前正式版仍为 `0.6.2`）：客户端与 lobby-service 版本号同步为 `0.6.3-alpha.2`。修复在客户端，lobby-service 仅版本号变化，不改 wire 协议，tail 房间 `minimumClientVersion` 保持 `0.6.2-alpha.1`。
...
- 修复 `0.6.2` 客户端 100% 无法加入「兼容旧版 Mod」（compat_4_5_v1）房间：加入流程曾无条件校验 join 响应的 `hostNativeBusTypeId`，而服务端对旧协议房间的既定合同从不携带该字段，导致加入后立即以 `lan_type_id_mismatch` 终止。现改为按 profile 校验：仅新协议（tail_v1）房间缺失/越界该字段才拒绝，旧协议房间不再要求；`protocolFlowNonce` 两种 profile 仍都必填（服务端对所有房间下发），tail_v1 的严格性不变。
- 修复新协议（tail_v1）房间里部分加入方看到房主名字显示为原版回退名 “Test Host” 的竞态：控制通道在 attach 之前预先连好时，房主广播的 `player_name_snapshot` 在加入方挂上处理程序之前到达而被静默丢弃，且此后不再有事件触发重播。现 attach 预连接控制通道后补发一次 `player_name_sync`（房主 upsert 后重播权威 snapshot），同时用 join 响应自带的房主名字先行播种本地名字目录（房主 ENet netId 恒为 1），两条路径互为兜底；随后到达的权威 snapshot 仍会整体覆盖播种值。
- 加入前 MOD 预检弹窗现在跟随大厅主题：此前切换到「午夜玻璃」或「SaaS 简洁」后，该弹窗及其列表行仍以街机配色与方角硬阴影弹出。街机主题保持原有外观不变；其他主题下弹窗改用不透明底色以保证小字可读，复选框改为随调色板着色（引擎默认的深灰图标在深色主题下几乎不可见），主按钮图标改用主按钮前景色。
- 预检弹窗在只剩「取消」一个可执行动作时不再并排显示两个相同的「取消」按钮。

### Added

- `scripts/publish-steam-workshop.sh`：创意工坊发布脚本。steamcmd 在独立的 `~/.steamcmd-home` 下运行，登录令牌不再被 Steam 桌面客户端重写 `config.vdf` 时清空；内容包与描述分两次推送，推送前拦截英文双引号、超过 1300 字节的字段与未包含当前版本号的描述，推送后经公开接口核对线上 manifest。
