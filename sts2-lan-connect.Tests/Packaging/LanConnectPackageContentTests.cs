...
public sealed class LanConnectPackageContentTests
{
    ...
    public async Task Client_package_uses_exact_allowlist_source_legal_bytes_and_deterministic_output()
    {
        ...
        Assert.Equal("0.5.6-rc2", manifest.RootElement.GetProperty("version").GetString());
        ...
        Assert.StartsWith("0.5.6-rc2", assemblyVersion.ProductVersion, StringComparison.Ordinal);
    ...
    public void Client_v053_release_documents_the_continue_channel_split_without_requiring_a_service_upgrade()
    {
        ...
        Assert.Contains("0.5.6-rc2", workshop, StringComparison.Ordinal);
        Assert.Contains("当前版本：0.5.6-rc2", workshop, StringComparison.Ordinal);
    ...
    [Fact]
    public void Client_v056_rc2_release_documents_field_report_fixes_and_candidate_state()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
        ...
        {
            Assert.Contains("0.5.6-rc2", text, StringComparison.Ordinal);
