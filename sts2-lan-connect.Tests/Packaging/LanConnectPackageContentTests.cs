...
public sealed class LanConnectPackageContentTests
{
    ...
    public async Task Client_package_uses_exact_allowlist_source_legal_bytes_and_deterministic_output()
    {
        ...
        Assert.Equal("0.6.1-alpha.2", manifest.RootElement.GetProperty("version").GetString());
        ...
        Assert.StartsWith("0.6.1-alpha.2", assemblyVersion.ProductVersion, StringComparison.Ordinal);
    ...
    public void Client_v053_release_documents_the_continue_channel_split_without_requiring_a_service_upgrade()
    {
        ...
        Assert.Contains("当前客户端：0.6.0；lobby-service：0.6.0", workshop, StringComparison.Ordinal);
    ...
    public void Client_v060_documents_stable_release()
    {
        ...
        Assert.Contains("| 客户端版本 | `0.6.0`（正式版） |", clientReadme, StringComparison.Ordinal);
        Assert.Contains("| Client version | `0.6.0` (stable) |", clientReadme, StringComparison.Ordinal);
        Assert.Contains("当前正式版为 `0.6.0`", userGuide, StringComparison.Ordinal);
