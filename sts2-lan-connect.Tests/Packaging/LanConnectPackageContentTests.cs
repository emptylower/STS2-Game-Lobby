...
public sealed class LanConnectPackageContentTests
{
    ...
    public async Task Client_package_uses_exact_allowlist_source_legal_bytes_and_deterministic_output()
    {
        ...
        Assert.Equal("0.6.3", manifest.RootElement.GetProperty("version").GetString());
        FileVersionInfo assemblyVersion = FileVersionInfo.GetVersionInfo(
            Path.Combine(packageDirectory, "sts2_lan_connect.dll"));
        Assert.Equal("0.6.3.0", assemblyVersion.FileVersion);
        Assert.Equal("0.6.3", assemblyVersion.ProductVersion?.Split('+')[0]);
    ...
    [Fact]
    public void Client_v053_release_documents_the_continue_channel_split_without_requiring_a_service_upgrade()
    {
        using Fixture fixture = new();
        ...
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.5.3_CLIENT_ZH.md"));
        ...
        foreach (string text in new[] { changelog, releaseNotes, clientReadme, userGuide })
        {
        ...
        Assert.Contains("续局身份码", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("lobby-service", releaseNotes, StringComparison.Ordinal);
    }
    ...
    public void Client_v060_alpha1_documents_dual_protocol_and_public_Ritsu_boundary()
    {
        ...
        Assert.DoesNotContain("ritsuTailBridge=True", releaseNotes, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Client_v060_alpha3_documents_post_connect_Ritsu_session_binding()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.0_ALPHA3_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));

        foreach (string text in new[] { releaseNotes, changelog })
        {
            Assert.Contains("0.6.0-alpha.3", text, StringComparison.Ordinal);
    ...
    [Fact]
    public void Client_v060_alpha5_documents_case_sensitive_capability_digest_fix()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.0_ALPHA5_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));

        foreach (string text in new[] { releaseNotes, changelog })
        {
            Assert.Contains("0.6.0-alpha.5", text, StringComparison.Ordinal);
        }
        foreach (string text in new[] { releaseNotes, changelog })
        {
            Assert.Contains("SL", text, StringComparison.Ordinal);
            Assert.Contains("WireCache", text, StringComparison.Ordinal);
        }
        Assert.Contains("Base64URL", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("0.6.0-alpha.4", releaseNotes, StringComparison.Ordinal);
    }

    [Fact]
    public void Client_v060_alpha6_documents_android_startup_and_save_dialog_fixes()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.0_ALPHA6_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));

        // Living docs (README, client install readme, user guide) follow the newest release;
        // their 0.6.0 content is asserted by Client_v060_documents_stable_release.
        foreach (string text in new[] { releaseNotes, changelog })
        {
        ...
        Assert.Contains("放弃多人存档", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("lobby-service", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("中继", releaseNotes, StringComparison.Ordinal);
    }

    [Fact]
    public void Client_v060_alpha7_documents_player_names_and_Ritsu_restart_reconnect()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.0_ALPHA7_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));
        ...
        foreach (string text in new[] { releaseNotes, changelog })
        {
            ...
            Assert.Contains("重开", text, StringComparison.Ordinal);
            Assert.Contains("RitsuLib", text, StringComparison.Ordinal);
        }
    ...
    public void Client_v060_alpha9_documents_lobby_missing_fix_candidate()
    {
        ...
        foreach (string text in new[] { releaseNotes, changelog })
        {
            Assert.Contains("0.6.0", text, StringComparison.Ordinal);
    ...
    [Fact]
    public void Client_v060_documents_stable_release()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.0_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));

        // Living docs (README, client install readme, user guide) follow the newest release;
        // their 0.6.0 content is asserted by Client_v061_documents_stable_release.
        foreach (string text in new[] { releaseNotes, changelog })
        {
            Assert.Contains("0.6.0", text, StringComparison.Ordinal);
            Assert.Contains("non_generic_v2", text, StringComparison.Ordinal);
        ...
        Assert.Contains("## [0.6.0] - ", changelog, StringComparison.Ordinal);
    }

    [Fact]
    public void Client_v061_documents_stable_release()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.1_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));
        ...
        string userGuide = File.ReadAllText(Path.Combine(
            ...
            "docs",
            "STS2_LAN_CONNECT_USER_GUIDE_ZH.md"));
        string workshopDescription = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "STEAM_WORKSHOP_DESCRIPTION_ZH.txt"));
        ...
        {
            Assert.Contains("0.6.1", text, StringComparison.Ordinal);
            Assert.Contains("native_bus_v1", text, StringComparison.Ordinal);
        }

        // The two feedback groups must survive into every packaged player-facing document.
        foreach (string text in new[] { releaseNotes, readme, clientReadme, userGuide, workshopDescription })
        {
            Assert.Contains("341498145", text, StringComparison.Ordinal);
            Assert.Contains("1093309523", text, StringComparison.Ordinal);
        }

        // The client install readme's version table and the user guide's "current stable" line are
        // living documents that follow the newest release; their current version text is asserted by
        // Client_v063_documents_stable_release. Only the 0.6.1 history section is pinned here.
        Assert.Contains("## v0.6.1 正式版", userGuide, StringComparison.Ordinal);
        Assert.DoesNotContain("Pre-release", clientReadme, StringComparison.Ordinal);
        Assert.DoesNotContain("Pre-release", userGuide, StringComparison.Ordinal);
    ...
    [Fact]
    public void Client_v062_documents_stable_release()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "RELEASE_NOTES_V0.6.2_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));
        string readme = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "README.md"));
        string clientReadme = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "CLIENT_RELEASE_README_ZH.md"));
        string announcement = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "UPDATE_ANNOUNCEMENT_V0.6.2_ZH.txt"));
        string workshopUpdate = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "STEAM_WORKSHOP_UPDATE_V0.6.2_ZH.txt"));
        string workshopDescription = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "STEAM_WORKSHOP_DESCRIPTION_ZH.txt"));

        foreach (string text in new[] { releaseNotes, changelog, readme, clientReadme, announcement, workshopUpdate })
        {
            Assert.Contains("0.6.2", text, StringComparison.Ordinal);
        }

        // The two feedback groups must survive into every packaged player-facing document.
        foreach (string text in new[] { releaseNotes, readme, clientReadme, announcement, workshopUpdate, workshopDescription })
        {
            Assert.Contains("341498145", text, StringComparison.Ordinal);
            Assert.Contains("1093309523", text, StringComparison.Ordinal);
        }

        // Current-version declarations are checked by Client_v063_documents_stable_release.
        string userGuide = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot,
            "docs",
            "STS2_LAN_CONNECT_USER_GUIDE_ZH.md"));
        Assert.Contains("## v0.6.2 正式版", userGuide, StringComparison.Ordinal);
        Assert.DoesNotContain("Pre-release", clientReadme, StringComparison.Ordinal);

        Assert.Contains("正式版", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("按对端寻址", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("服务端版本过旧", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("sts2_lobby_service.zip", releaseNotes, StringComparison.Ordinal);
        Assert.Contains("Steam 创意工坊", releaseNotes, StringComparison.Ordinal);
        Assert.DoesNotContain("pre-release", announcement, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("## [0.6.2] - ", changelog, StringComparison.Ordinal);
    }

    [Fact]
    public void Client_v063_documents_stable_release()
    {
        using Fixture fixture = new();
        string releaseNotes = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot, "docs", "RELEASE_NOTES_V0.6.3_ZH.md"));
        string readme = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "README.md"));
        string clientReadme = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot, "docs", "CLIENT_RELEASE_README_ZH.md"));
        string userGuide = File.ReadAllText(Path.Combine(
            fixture.RepositoryRoot, "docs", "STS2_LAN_CONNECT_USER_GUIDE_ZH.md"));
        string changelog = File.ReadAllText(Path.Combine(fixture.RepositoryRoot, "CHANGELOG.md"));

        Assert.Contains("| 客户端版本 | `0.6.3`（正式版） |", clientReadme, StringComparison.Ordinal);
        Assert.Contains("| Client version | `0.6.3` (stable) |", clientReadme, StringComparison.Ordinal);
        Assert.Contains("当前正式版为 **v0.6.3**", readme, StringComparison.Ordinal);
        Assert.Contains("当前正式版为 `0.6.3`", userGuide, StringComparison.Ordinal);
    ...
    public void Client_v060_alpha8_documents_android_non_generic_diagnostic_candidate()
    {
        ...
        // README, the client install readme, and the user guide are living documents that follow
        // the newest release; their 0.6.0 content is asserted by Client_v060_documents_stable_release.
        foreach (string text in new[] { releaseNotes, changelog })
        {
