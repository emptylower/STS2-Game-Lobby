...
| (可选) 公共列表服务源码 | `server-registry/` | v0.3.x 时代的母面板源码；v0.5.2 不再需要，仅供想自托管列表服务的运维参考 |
...
- v0.5.2 服务端继续兼容 v0.5.1 聊天与 MOD 同步协议，不改变加入流程
...
4. (可选) 想自托管完整公共列表服务时再看 [`server-registry/README.md`](./server-registry/README.md)（v0.5.2 不依赖它）
...
| [`docs/STS2_LOBBY_DEPLOYMENT_GUIDE_ZH.md`](./docs/STS2_LOBBY_DEPLOYMENT_GUIDE_ZH.md) | 当前中文部署主路径（v0.5.2） |
| [`docs/STS2_SERVER_DOCKER_OPERATION_GUIDE_ZH.md`](./docs/STS2_SERVER_DOCKER_OPERATION_GUIDE_ZH.md) | Docker 部署与运维指南（v0.5.2 单容器 + v0.3.x 双服务栈兼容路径） |
...
| (Optional) Public listing service source | `server-registry/` | Source for v0.3.x-style self-hosted public listing service; not required in v0.5.2 |
...
- The v0.5.2 service retains the v0.5.1 chat and MOD-sync protocols and existing join behavior.
...
4. (Optional) [`server-registry/README.md`](./server-registry/README.md) only if you want to self-host the v0.3.x-style public listing service — v0.5.2 itself does not require it
