...
- 大厅邀请功能：房主在准备页面点击"大厅邀请"按钮，自动生成包含服务器地址、房间 ID 和密码的邀请码并复制到剪贴板；其他玩家复制邀请码后打开大厅，会自动检测剪贴板并弹出确认对话框，一键加入（支持跨服务器自动切换）
...
- `4` 人大厅房间会自动发布为 `legacy_4p` 兼容协议，可与 `0.2.2` 联机；`5-8` 人房间会发布为 `extended_8p`，仅支持 `0.2.3+`
...
- 加入时间过长时，进度弹窗右上角会出现取消按钮；用户手动取消后不会继续向房主发送连接申请
